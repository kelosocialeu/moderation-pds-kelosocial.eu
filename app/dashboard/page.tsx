...
async function getReports(): Promise<Report[]> {
  ...
  try {
    ...
    if (
      ...
      return (data as { reports: Report[] }).reports
    }
  ...
  return (
    <main className="shell">
      ...
      <section className="content">
        <div className="top">
          <div>
            ...
            <p className="subtitle">
              Une console pensée pour examiner les signalements, conserver une trace des décisions et agir sans exposer les secrets du PDS au navigateur.
            </p>
          </div>
        ...
        <div className="grid">
          <div className="card">
            <div className="label">Signalements ouverts</div>
            <div className="metric">{reports.length}</div>
          </div>
          <div className="card">
            <div className="label">Priorité haute</div>
            <div className="metric">{reports.filter((r: Report) => r.severity === 'high').length}</div>
          </div>
          <div className="card">
            <div className="label">Décisions aujourd’hui</div>
            <div className="metric">—</div>
          </div>
          <div className="card">
            <div className="label">État du service</div>
            <div className="metric" style={{ fontSize: 18, color: 'var(--ok)' }}>
              {process.env.MODERATION_API_URL ? 'Connecté' : 'À connecter'}
            </div>
          </div>
        </div>
        ...
        <section className="card queue">
          ...
          {reports.length === 0 ? (
            <p className="subtitle">
              Aucun signalement remonté pour le moment. Le tableau est prêt à être relié au service de modération AT Protocol Kelo.
            </p>
          ) : (
            reports.map((r: Report) => (
              <article className="report" key={r.id}>
                <div>
                  <div className="reason">{r.reason || 'Signalement'}</div>
                  <div className="meta">
                    {r.subject} {r.createdAt ? `• ${r.createdAt}` : ''}
                  </div>
                </div>
